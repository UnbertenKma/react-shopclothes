        ...
        addItem: (state, action) => {
            ...
            if (duplicate.length > 0) {
                ...
                state.value = [...state.value, {
                    ...newItem,
                    id: duplicate[0].id,
                    quanlity: newItem.quanlity + duplicate[0].quantity
                }]
            } else {
                state.value = [...state.value, {
                    ...newItem,
...
const findItem = (arr, item) => arr.filter(e => e.slug === item.slug && e.color === item.color && e.
size === item.size)

const delItem = (arr, item) => arr.filter(e => e.slug !== item.slug || e.color !== item.color || e.
size !== item.size)
